const reviews = [
  {
    ...
    confidence: 'high', verified: true, confidenceLabel: 'High confidence', signals: [['Procedure mentioned', 'Specific'], ['Surgeon named', 'Matched'], ['Recovery detail', '7 days'], ['Source type', 'Patient review']], source: 'Naver Cafe · sample', note: 'The clinic and surgeon fields were matched against the guide’s directory. This is fictional sample data for the prototype.'
  },
  {
  ...
  },
  {
    ...
    confidence: 'high', verified: true, confidenceLabel: 'High confidence', signals: [['Procedure mentioned', 'Laser toning'], ['Price detail', 'Itemised'], ['Recovery detail', 'None'], ['Source type', 'Patient review']], source: 'Naver blog · sample', note: 'Price information is time-sensitive and should be confirmed with the clinic before travel.'
  },
  {
    ...
    confidence: 'medium', verified: false, confidenceLabel: 'Review checked', signals: [['Procedure mentioned', 'General'], ['Surgeon named', 'Matched'], ['Recovery detail', 'Partial'], ['Source type', 'Patient review']], source: 'Naver Cafe · sample', note: 'Advice from a reviewer is not medical advice. The detail is surfaced as a question prompt.'
  },
  {
    ...
    confidence: 'high', verified: true, confidenceLabel: 'High confidence', signals: [['Procedure mentioned', 'Specific'], ['Price detail', 'Not listed'], ['Recovery detail', 'Logged'], ['Source type', 'Patient review']], source: 'Naver blog · sample', note: 'The record is useful for logistics. It does not establish that a treatment is suitable for any individual.'
  },
  {
    ...
    confidence: 'medium', verified: false, confidenceLabel: 'Needs follow-up', signals: [['Procedure mentioned', 'Specific'], ['Surgeon named', 'Not listed'], ['Recovery detail', '1 month'], ['Source type', 'Patient review']], source: 'Naver Cafe · sample', note: 'A single patient account cannot establish frequency or causation. The concern is shown to prompt a clinician question.'
  }
];
...
function filteredReviews() {
  ...
  return reviews.filter(review => {
    ...
    const matchesTrust = trust === 'all' || (trust === 'verified' ? review.verified : review.confidence === 'high');
